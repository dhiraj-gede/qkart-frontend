...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const response = await axios.post(`${config.endpoint}/user/addresses`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const performCheckout = async (token, items, addresses) => {
    const validate = validateRequest(items,addresses)
    if (!validate) return
    try {
      const payload = {"addressId":addresses.selected._id}
      const response = await axios.post(`${config.endpoint}/cart/checkout`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      const balance = localStorage.getItem('balance') - getTotalCartValue(items)
    ...
    <>
      ...
      <Grid container>
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
                ...
                <React.Fragment>
                  {addresses.all.map(item => {
                    if (addresses.selected._id === item._id) {
                      return (<Box className="address-item selected" key={item._id} onClick={() => {
                        setAddresses({selected:item,all:addresses.all})
                      }}>
                        {item.address}
                        <Button startIcon={<Delete />} onClick={() => deleteAddress(token,item._id)}>DELETE</Button>
                      </Box> )
                    }

                    return (<Box className="address-item not-selected" key={item._id} onClick={() => {
                      setAddresses({selected:item,all:addresses.all})
                    }}>
                      {item.address}
                      <Button startIcon={<Delete />} onClick={() => deleteAddress(token,item._id)}>DELETE</Button>
        ...
        <Grid container justifyContent="center" item xs={3} 
          sx={{backgroundColor:"#E9F5E1",padding:0.5,alignContent:"flex-start"}}>
